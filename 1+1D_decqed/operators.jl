...
function sinekleingordon_dec(dx::Float64, dt::Float64, nv_x::Int64, phix_current::Array{Float64,1}, phit_past::Array{Float64,1},
                          phi_current::Array{Float64,1}, rhs::Array{Float64,1}, qq::Array{Float64,1}, alpha::Float64, bctype::String, g::Float64)
    ...
    tempr2 = 1-alpha*dt/2
    for v = 2:nv_x-1 # exclude the boundary points
        phit_current[v] = tempr2/tempr1*phit_past[v] + dt^2/tempr1*( (phix_current[v]-phix_current[v-1])/dx^2 - rhs[v] - qq[v]*sin(phi_current[v]) - g^2*phi_current[v] )
    end 
    
    if bctype == "periodic"
       phit_current[1] = tempr2/tempr1*phit_past[1] + dt^2/tempr1*( (phix_current[1]-phix_current[nv_x-1])/dx^2 - rhs[1] - qq[1]*sin(phi_current[1]) - g^2*phi_current[v] )
       phit_current[nv_x] = phit_current[1]
    end
