...
for t = 1:nv_t-1
    ...
    phit_current = kleingordon_massive_dec(dx, dt, nv_x, phix_current, phit_past, phi_current, rhs, g, m);
...
#myplot = heatmap(-(nv_x-1)/2*dx:dx:(nv_x-1)/2*dx, dt:dt:nv_t*dt, phi, xlabel =("x"), ylabel = ("t"), title = "\$\\phi\$", clims = (-color_phi, color_phi));
...
########################## 
###@save "SimulationData.jld2" phi phix phit W g L Tm dx dt Q nv_x nv_t Nv energycharge energycurrent energyfield # might exceed disc quota if arrays are large
###@save "SimulationData.jld2" phi W g m L Tm dx dt Q nv_x nv_t Nv energycharge energycurrent energyfield # only saves phi for high-res calculations
...
savefig(myplot,"phi_vs_t_at_x=100_Tm2000.png");
...
savefig(myplot,"phit_vs_t_at_x=100_Tm2000.png");
...
savefig(myplot,"phi_vs_t_at_x=0_Tm2000.png");

myplot = plot(t, current_at0[startcut:endcut], title = "\$J_x(t)\$ at \$x=0\$", xlabel="t", ylabel="\$J_x\$");
savefig(myplot,"current_vs_t_at_x=0_Tm2000.png");

myplot = plot(t, efield_at0[startcut:endcut], title = "\$E(t)\$ at \$x=0\$", xlabel="t", ylabel="\$E\$");
savefig(myplot,"efield_vs_t_at_x=0_Tm2000.png");


@save "results_at_x=100_and_x=0_m0.jld2" W g L Tm dx dt Q m nv_x nv_t Nv tempphi_vstime_list temp_phit_vstime_list temp_phix_vstime_list current_at0 efield_at0 phi_at0
